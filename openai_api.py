...
def chat_completion_request(messages, tools=None, tool_choice=None, model=config.OPENAI_MODEL):
    new_messages = []
...
@retry(wait=wait_random_exponential(multiplier=1, max=40), stop=stop_after_attempt(3))
def format_sql_response(sql_response: str, model: str = config.OPENAI_MODEL) -> str:
